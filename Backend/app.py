...
def start_scraper():
    ...
    scraper_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Scraper')
    scraper_path = os.path.join(scraper_directory, 'scraper.py')

    command = ['python', scraper_path, url, search_text, '/results']
    subprocess.Popen(command, cwd=scraper_directory)
...
def update_tracked_products():
    ...
    for tracked_product in tracked_products:
        ...
        command = ['python', scraper_path, url, name, '/results']
        subprocess.Popen(command, cwd=scraper_directory)
